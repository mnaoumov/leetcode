...
/// <summary>
/// https://leetcode.com/submissions/detail/934391356/
/// </summary>
[UsedImplicitly]
public class Solution2 : ISolution
{
    public IList<string> GenerateSentences(IList<IList<string>> synonyms, string text)
    {
        ...
        var comparer = Comparer<string>.Create((s1, s2) => StringComparer.Ordinal.Compare(s1, s2));

        foreach (var synonym in synonyms)
        {
            var key = uf.Find(synonym[0]);
            setsMap.TryAdd(key, new SortedSet<string>(comparer));

...
public class Solution5 : ISolution
{
    ...
    private static List<int> Add(IReadOnlyList<int> digits1, IReadOnlyList<int> digits2)
    {
    ...
    private static int GetDigitFromEnd(IReadOnlyList<int> digits, int indexFromEnd) => indexFromEnd < digits.Count ? digits[digits.Count - 1 - indexFromEnd] : 0;

    private static List<int> Multiply(IEnumerable<int> digits1, int digit2)
    {
        var resultDigits = new List<int>();

        var curry = 0;

        foreach (var digit1 in digits1.Reverse())
        {

...
/// <summary>
/// https://leetcode.com/submissions/detail/963948069/
/// </summary>
[UsedImplicitly]
...
public class Solution4 : ISolution
{
    ...
    private class Trie
    {
        ...
        public class Node
        {
            ...
            private Node?[]? _letterNodes;

            public Node GetOrAddLetterNode(char letter)
            {
                _letterNodes ??= new Node?[LettersCount];
                return GetLetterNode(letter) ?? (_letterNodes[ToIndex(letter)] = new Node());
            }

            public void MarkAsEndOfWord(int index) => WordIndex = index;
            public Node? GetLetterNode(char letter) => _letterNodes?[ToIndex(letter)];
            private static int ToIndex(char letter) => letter - 'a';
        }
    }
}

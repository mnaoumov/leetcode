...
/// <summary>
/// https://leetcode.com/problems/analyze-user-website-visit-pattern/submissions/1778594135/
/// </summary>
[UsedImplicitly]
public class Solution2 : ISolution
{
    public IList<string> MostVisitedPattern(string[] username, int[] timestamp, string[] website)
    {
        ...
        var entriesByUsers = entries.GroupBy(e => e.UserName).Select(g => g.ToArray()).ToArray();

        var counts = new Dictionary<string, int>();

        foreach (var userEntries in entriesByUsers)
        {
            var m = userEntries.Length;
            var userPatterns = new HashSet<string>();
            ...
            for (var i = 0; i < m; i++)
            {
                for (var j = i + 1; j < m; j++)
                {
                    for (var k = j + 1; k < m; k++)
                    {
                        ...
                        if (!userPatterns.Add(patternStr))
                        {
                            continue;
                        }

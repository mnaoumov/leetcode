...
public class Solution2 : ISolution
{
    public int Ways(string[] pizza, int k)
    {
        ...
        appleCountsDp.GetOrCalculate((0, 0));
        ...
        var dp = new DynamicProgramming<(int row, int column, int peopleCount), int>((key, recursiveFunc) =>
        {
            ...
            {
                if (appleCountsDp.GetOrCalculate((row, column)) == appleCountsDp.GetOrCalculate((cutRow + 1, column)))
                {
                    continue;
                }
            ...
            {
                if (appleCountsDp.GetOrCalculate((row, column)) == appleCountsDp.GetOrCalculate((row, cutColumn + 1)))
                {
                    continue;
                }

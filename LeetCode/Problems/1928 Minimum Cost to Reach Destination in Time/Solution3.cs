...
/// <summary>
/// https://leetcode.com/submissions/detail/941147849/
/// </summary>
[UsedImplicitly]
...
public class Solution3 : ISolution
{
    public int MinCost(int maxTime, int[][] edges, int[] passingFees)
    {
        ...
        var fees = new Dictionary<int, int>();

        Dfs(0, 0, beforeStartCity, 0);
        ...
        void Dfs(int city, int time, int previousCity, int previousFee)
        {
            if (time > maxTime)
            {
                return;
            ...
            var fee = previousFee + passingFees[city];

            if (fees.GetValueOrDefault(city, int.MaxValue) < fee)
            {
                return;
            }
            ...
            {
                Dfs(road.Other(city), time + (int) road.Weight, city, fee);
            }
        }
    }

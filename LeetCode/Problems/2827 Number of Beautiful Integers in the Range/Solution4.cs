...
/// <summary>
/// https://leetcode.com/submissions/detail/1025969817/
/// </summary>
[UsedImplicitly]
        ...
        var dp = new DynamicProgramming<(int max, int evenOddBalance, int remainder, bool allowLeadingZero), int>(
            (key, recursiveFunc) =>
            {
                ...
                while (firstDigit >= 10)
                {
                    ...
                    withoutFirstDigit += lastDigit * powerOfTen;
                    firstDigit /= 10;
                    powerOfTen *= 10;
                    digitsCount++;
                }
                ...
                for (var digit = 0; digit <= firstDigit; digit++)
                {
                    ...
                    var zerosCount = digitsCount - nextMax.ToString().Length + (nextMax == 0 ? 1 : 0);
                    var nextEvenOddBalance = evenOddBalance + (digit == 0 && !allowLeadingZero
                        ? 0
                        : (digit % 2 == 0 ? 1 : -1) + zerosCount);
                    var nextRemainder = (k + remainder - digit * powerOfTen % k) % k;
                    var nextAllowLeadingZero = allowLeadingZero || digit != 0;

                    if (nextMax == 0)
                    {
                        if (nextRemainder == 0 && nextEvenOddBalance == 0 && nextAllowLeadingZero)
                        {
                            ans++;
                        }
                    }

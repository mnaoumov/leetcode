...
/// <summary>
/// https://leetcode.com/contest/weekly-contest-359/submissions/detail/1026338981/
/// </summary>
[UsedImplicitly]
...
public class Solution1 : ISolution
{
    public int LongestEqualSubarray(IList<int> nums, int k)
    {
        ...
        var dp = new DynamicProgramming<(int index, int removalCount), int>((key, recursiveFunc) =>
        {
            ...
            var nextRemovalCount = removalCount - (nextIndex - index - 1);
            return nextIndex < n && nextRemovalCount >= 0 ? 1 + recursiveFunc((nextIndex, nextRemovalCount)) : 1;
        });

        return Enumerable.Range(0, n).Max(index => dp.GetOrCalculate((index, k)));
    }

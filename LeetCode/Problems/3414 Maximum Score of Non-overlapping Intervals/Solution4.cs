...
/// <summary>
/// https://leetcode.com/problems/maximum-score-of-non-overlapping-intervals/submissions/2141108368/
/// </summary>
[UsedImplicitly]
public class Solution4 : ISolution
{
    private const int MaxIntervalCount = 4;
    ...
    public int[] MaximumWeight(IList<IList<int>> intervals)
    {
        ...
        var lefts = sortedIntervals.Select(x => x.Left).ToArray();

        var nextIndices = new int[n];

        for (var i = 0; i < n; i++)
        {
            nextIndices[i] = FindFirstIndexWithLeftGreaterThan(lefts, sortedIntervals[i].Right);
        }
        ...
        for (var minIndex = n; minIndex >= 0; minIndex--)
        {
            for (var maxCount = 0; maxCount <= MaxIntervalCount; maxCount++)
            {
                dp.GetOrCalculate((minIndex, maxCount));
            }
        }

        return dp.GetOrCalculate((0, MaxIntervalCount)).indices;
    }

    private static int FindFirstIndexWithLeftGreaterThan(int[] lefts, int value)
    {
        var low = 0;
    ...
    private sealed record Interval(int Left, int Right, int Weight, int OriginalIndex);
}

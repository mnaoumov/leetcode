...
/// <summary>
/// https://leetcode.com/problems/minimum-cost-path-with-teleportations/submissions/1899306917/
/// </summary>
[UsedImplicitly]
...
public class Solution5 : ISolution
{
    public int MinCost(int[][] grid, int k)
    {
        ...
        var costDp = new Dictionary<(int cost, int teleportsLeft), int>();
        var finalCost = grid[m - 1][n - 1];
        ...
        var dp = new DynamicProgramming<(int row, int column, int teleportsLeft), int>((key, getOrCalculate) =>
        {
            ...
            if (teleportsLeft > 0 && grid[row][column] >= finalCost)
            {
                return 0;
            }
            ...
            if (teleportsLeft > 0)
            {
                ans = Math.Min(ans, costDp[(grid[row][column], teleportsLeft - 1)]);
            }
        ...
        for (var teleportsLeft = 0; teleportsLeft < k; teleportsLeft++)
        {
            ...
            {
                if (cost >= finalCost && teleportsLeft > 0)
                {
                    costDp[(cost, teleportsLeft)] = 0;
                    continue;
                }

...
/// <summary>
/// https://leetcode.com/contest/biweekly-contest-177/problems/minimum-operations-to-make-array-parity-alternating/submissions/1933820616/
/// </summary>
[UsedImplicitly]
        ...
        else
        {
            for (var initialParity = 0; initialParity <= 1; initialParity++)
            {
                ...
                foreach (var deltaMin in new[] { -1, 1 })
                {
                    ...
                    {
                        if (numsMatchingParityByInitialParity[initialParity ^ 1].Count == 1 && deltaMin != deltaMax)
                        {
                            continue;
                        }
